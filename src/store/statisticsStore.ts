    ...
    (set, get) => ({
      ...
      updateFromInvoice: (invoice: Invoice, previousStatus?: string) => {
        ...
        const isCurrentMonth =
          startOfMonth(invoiceDate).getTime() ===
          startOfMonth(currentDate).getTime();
        const isCurrentYear =
          startOfYear(invoiceDate).getTime() ===
          startOfYear(currentDate).getTime();
        ...
        set((state) => {
          ...
          if (previousStatus !== "paid" && invoice.status === "paid") {
            ...
            const existingMonth = newHistory.find(
