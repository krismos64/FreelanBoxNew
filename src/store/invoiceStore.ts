    ...
    (set, get) => ({
      ...
      updateInvoice: (id, updatedInvoice) => {
        ...
        if (oldInvoice && updatedInvoice.status) {
          useStatisticsStore.getState().updateFromInvoice(
        ...
        if (oldInvoice && updatedInvoice.status) {
          ...
          if (client) {
            ...
            } else if (oldInvoice.status === 'paid' && updatedInvoice.status !== 'paid') {
              clientStore.updateClient(client.id, {
      ...
      deleteInvoices: (ids) => {
        ...
        const invoicesToDelete = get().invoices.filter(inv => ids.includes(inv.id));
